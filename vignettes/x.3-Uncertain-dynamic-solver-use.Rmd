...
The first step is to determine the number of uncertain variables and the number of runs. We also need the SB-variable to convert mass (result of the solvers) to concentration in units associated by the compartments

```{r Create samples, warning=FALSE, message=FALSE}
ConcDividerVar <- World$NewCalcVariable("Mass2ConcDivider")
# We keep the solver object, but for normal use, apply the "World" methods / properties; the new solver is part of it!
...
Boxvars <- Boxvars |>
  mutate(
    ...
    b = waarde*1.5,
    c = waarde) |>
  mutate(Distribution = "triangular") |>
  select(-waarde)
...
```{r Test steady state, warning=FALSE, message=FALSE}

n_samples <- 2 # The number of samples you want to pull from the distributions for each variable
World$NewSolver("SteadyODE")

World$Solve(emissions = emisFuns, var_box_df = Boxvars, var_invFun = varFuns)
...
World$Solve(emissions = emisFuns, tmax = tmax, nTIMES = nTIMES, uncertain_emis = F, var_box_df = Boxvars, var_invFun = varFuns, nRUNs = n_samples)
...
If you think your variables are correlated you can use ?? TODO, via normal ?? ask SIM:
```{r}
