...
SB1solver <- World$NewSolver("SB1Solve")
```
...
varFuns <- apply(Boxvars, 1, function(aRow){ 
  prepArgs <- as.list(as.numeric(aRow[c("a", "b", "waarde")]))
  names(prepArgs) <- c("a", "b", "c") 
  #now, with all arguments properly prepared, we can
  World$Make_inv_unif01(fun_type = "triangular", pars = prepArgs)
})
...
World$Solve(emissions = emisFuns, var_box_df = Boxvars, var_invFun = varFuns, nRUNs = n_samples)
