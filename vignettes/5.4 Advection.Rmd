---
title: "5.4 Advection"
author: "Jaap Slootweg, Joris Quik, Valerie de Rijk, Anne Hids"
date: "`r Sys.Date()`"
...
![Overview of water flows between Regional and Continental scale.
<!--# origin: SBcoreModules.pptx -->](Figures_Custom/x_Reg_Cont_flow_overview.png)

![Overview of water flows at Global scale.
<!--# origin: SBcoreModules.pptx -->](Figures_Custom/x_GlobalScale.png)
...
When the Regional_and_Cohntinental_deepocean variable == TRUE, this function is 
also used to calculate the flow between deepocean at Regional and Continental scale. 
...
currents based on a set ocean current of
`r World$fetchData("OceanCurrent")/(60*60*24)` m3/day.
...
## Remove global scales

The variable remove_global removes the global (Arctic, Moderate and
Tropic) scales from the model. When this variable is set to TRUE,
advection flows to, from and within these scales are removed. The use of
this variable is demonstrated below.

```{R}
World$SetConst(Remove_global = "TRUE") 
World$UpdateKaas(mergeExisting = FALSE)
```

## Remove Regional and Continental deepocean

The deepocean compartment and Regional and Continental scale can be
introduced by setting the variable Regional_and_Continental_deepocean to
TRUE. When this variable is TRUE, the following changes are made:
...
- There is no advection between Regional sea and Continental sea. 
- Advection flows between 
```
...
## Overwrite advection flows with AdvInput

The variable 'AdvInput' was added to SimpleBox. It allows the user to
overwrite the advection flows from a compartment at a scale with a user
defined value. These values can be given in ScaleSubCompartData.csv.
