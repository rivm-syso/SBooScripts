...
*Data from an DPMFA model should be prepared to fit the SBoo world. For instance the time unit should be correct, the mass unit is not as important as this will be the same in the output then, but for good measure we use kg.* Emissions seem to not be right.
...
Emissions <-
  ...
  summarise(n=n(),
            EmisFun = list(approxfun(
              data.frame(time_s = c(0,time_s), emis_kg=c(0,Emission_mean_kg)),rule = 1:2)))
...
SBtime <- times <- seq(0, 24*365.25*24*360, by = 10000)
results <- lapply(funlist, function(f) f(SBtime))

# Convert results to a data frame for plotting
df <- data.frame(
  Time = rep(SBtime, length(funlist)),
  Value = unlist(results),
  Function = rep(names(funlist), each = length(SBtime))
)

# Plot the results
ggplot(df, aes(x = Time, y = Value, color = Function)) +
  geom_line() +
  labs(title = "Function Evaluations Over Time",
       x = "Time (seconds)",
       y = "Function Value") +
  theme_minimal()
...
for (i in seq_along(engine_results)) {
  ...
  SBtime <- seq(0, 365.25 * 24 * 60 * 60 * 1, length.out = 12)
...
```{r Output Processing }
Solution <- as.data.frame(Solutions[[1]])
print(NamesK)
print(length(NamesK))
...
dfs <- list(A = A, C = C, T = T, R = R, M = M)
titles <- c("A", "C", "T", "R", "M")

# Loop through the list and plot each dataframe
for (i in seq_along(dfs)) {
  plot <- plot_dataframe(dfs[[i]], titles[i])
  print(plot)
}
