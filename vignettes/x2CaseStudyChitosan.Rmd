...
We calculate the spherical equivalent diameter (deq) and subsequently use it to overwrite radS. In this manner we include the shape of the considered particles. We update the matrix in the chunk after.

We assume the following things for Chitosan: 
- Graphene oxide has a diameter of roughly 70um 
- Chitosan of approximately 10 um
- GO-chitosan of 80um 
...
Since attachment efficiences (alpha) are very uncertain, below is a chunk where we can create distributions for these parameters. For the deterministic run we
...
### Scaling Input Data based on Density
...
# Weightfactor <- 7/8 
# Emissions <- 
#   Emissions |> mutate(Emissions$Emission_mean_kg , Emission_mean_kg* Weightfactor)
```


### Scaling Input Data based on World
