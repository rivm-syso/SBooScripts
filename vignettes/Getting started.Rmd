...
We can now plot the masses and concentrations using the built-in plot functions. 
The functions are called by using `World$PlotSolution` for masses, and `World$PlotConcentration`
for concentrations. You need to specify which scale you want to plot the outcome for. If no 
scale is specified, the Regional scale will be chosen for you. Additionally you can define 
for which SubCompart(s) the outcomes should be plotted. If you don't specify this, the outcomes
will be plotted for all SubComparts in the Scale. 
