...
write.xlsx(output_table, "/rivm/r/E121554 LEON-T/03 - uitvoering WP3/Recipe/FF_output_table_SS.xlsx")
```
...
Dyn_masses_clean <- Dyn_masses_continental |>
  ...
  filter(EqMass_SAP >= p05 & EqMass_SAP <= p95) |>
  select(-p05, -p95) |> # Remove the percentile columns after filtering
  ungroup()
```

```{r Make table, warning=FALSE, message=FALSE}
output_table <- Dyn_masses_clean |>
  group_by(Polymer,CompartmentFF,EmisComp, Year) |> 
...
write.xlsx(output_table, "/rivm/r/E121554 LEON-T/03 - uitvoering WP3/Recipe/FF_output_table_Dyn.xlsx")
```
