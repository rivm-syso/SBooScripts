...
substance <- "Perfluorobutane_sulfonic"
...
write.csv(emissions, file = "vignettes/CaseStudies/PFAS Tjebbe/Data/20250312_emissions.csv")


emissions2 <-
  emissions |> rbind(data.frame(
    Emis = 0, 
    Abbr = "aRU",
    Time = seq(from = 2209032000+31557600, 
               to = 2209032000+70*(31557600), by = 31557600)
  ))

World$fetchData("kdeg") |> 
  mutate(
    DegHalfLife_years = log(2) / kdeg / (365.25*24*60*60) # ik zou verwachten veel kdeg van jaren.
  )
...
``` {r Solve}
#Mutating:
# df <- data.frame(varName = "kfrag",
#                     Waarde = 0.000000027)
# World$mutateVars(df)
# World$UpdateDirty("kfrag")
# World$fetchData("RadS")

#New solver
World$NewSolver("DynamicSolver")
World$Solve(emissions = emissions2, tmax = tmax, nTIMES = nTIMES)
...
World$PlotMasses(scale = "Regional", subcompart = "sea")
