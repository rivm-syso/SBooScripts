...
receiving_compartments <- c("aRS","w1RS","w0RS","w2RS","sd1RS","sd0RS","sd2RS","s1RS","s2RS",
                            "aCS","w1CS","w0CS","w2CS","sd1CS","sd0CS","sd2CS","s1CS","s2CS") 
...
eef_compartments <- c(EEF_air, EEF_w, EEF_w, EEF_w, EEF_sed, EEF_sed, EEF_sed, EEF_s, EEF_s,
                      EEF_air, EEF_w, EEF_w, EEF_w, EEF_sed, EEF_sed, EEF_sed, EEF_s, EEF_s)

### fraction of species in each compartment
FracSpe_wc_marine = 0.4458 #fraction of marine species feeding in the water column - Saadi et al., 2025
FracSpe_sed_marine = 0.5542 #fraction of marine species feeding in the sediment - Saadi et al., 2025
...
for(reg in region_names){
  ...
  total <- SDF_AE_lw_C + SDF_AE_lw_sed_C + SDF_AE_fw_C + SDF_AE_fw_sed_C +
    ...
    SDF_ME_sw_C + SDF_ME_sw_sed_C + SDF_ME_sw_G + SDF_ME_sw_sed_G + #marine SDF = 1
  ...
  tab_SDF <- matrix(0, nrow = 3, ncol = 18)
  
  # Name rows and columns
  rownames(tab_SDF) <- c("Marine_Ecosystem", "Freshwater_Ecosystem", "Terrestrial_Ecosystem")
  colnames(tab_SDF) <- c("a_C","fw_C","lw_C",  "sw_C", "fw_sed_C","lw_sed_C",  "sw_sed_C",     #the order is slightly different than in Louvet et al. 20205
                         "nat_soil_C", "agr_soil_C", "a_G","fw_G","lw_G",  "sw_G", "fw_sed_G", 
                         "lw_sed_G", "sw_sed_G", "nat_soil_G", "agr_soil_G")
  # Fill the matrix with the SDFs
  SDF <- as.data.frame(tab_SDF)
  # Marine ecosystem
  SDF["Marine_Ecosystem", "sw_C"]       <- SDF_ME_sw_C
  SDF["Marine_Ecosystem", "sw_sed_C"]   <- SDF_ME_sw_sed_C
  SDF["Marine_Ecosystem", "sw_G"]       <- SDF_ME_sw_G
        ...
        for (emission_compartment in emission_compartments) {
          ...
          Masses_grouped_over_species <- Masses |>
            ...
            mutate( #reorder the resulting df
              ...
              SubCompart = factor(SubCompart, levels = c("air","river","lake","sea","freshwatersediment","lakesediment","marinesediment","naturalsoil","agriculturalsoil")) #match the list refined for emission/receiving compartments
            ) |>
          ...
          CF_end_PDF_m2_year <- Masses_grouped_over_species |> 
            ...
            mutate(SubCompart = ifelse(Scale %in% c("Regional", "Continental") & SubCompart %in% c("sea", "marinesediment"),
