...
##Emission data
Now, we prepare our emission data to be used as input. We create approxfuns based on the input data. These will serve as input data the dynamic uncertain solver.
```{r emission data}
file_paths <- 
...
nruns <- nrow(Emis_nested_GO_chit$Emis[[1]])
lhs_samples <- optimumLHS(nruns, 1)
...
species <- c("Small", "Large")

scaled_samples_fw <- tibble(varName = "alpha",
                            Species = "any",
                            SubCompart = "FreshWater",
                            value = fw_alpha_values[, 1],
                            RUN = 1:(nruns))

scaled_samples_ma <- tibble(varName = "alpha",
                            Species = "any",
                            SubCompart = "Marine",
                            value = ma_alpha_values[, 1],
                            RUN = 0:(nruns-1))

Sample_df <- bind_rows(scaled_samples_fw, scaled_samples_ma)
...
Sample_df <- explodeF(Sample_df, "SubCompart", "FreshWater", subcomparts)  
Sample_df <- explodeF(Sample_df, "SubCompart", "Marine", subcompartsmarine)  
Sample_df <- explodeF(Sample_df, "Species", "any", species) 

Sample_df_nested <- Sample_df |>
  mutate(Scale = NA) |>
  group_by(varName, SubCompart, Species, Scale) |>
  nest(data = c(RUN, value))
```
...
Concentration_f <- function(dataframe){
  Concentration_long <- 
  dataframe |>
  ...
  ungroup() |> 
  group_by(Year, Scale, SubCompart, RUN) |>
...
```{r Solve with uncertain dynamic solver}
World$NewSolver("UncertainDynamicSolver")

Solution_GO_chit <- World$Solve(Emis_nested_GO_chit, Sample_df_nested, needdebug = F,
                        rtol_ode=1e-30, atol_ode = 0.5e-2)

Solution_long_GO_chit <- Solution$DynamicMass |>
  select(!starts_with("emis")) |>
  pivot_longer(!c(time, RUN), names_to = "Abbr", values_to = "Mass") |>
  left_join(World$states$asDataFrame, by = "Abbr") |>
  mutate(Year = time/(365.25*24*60*60))

Concentration_long_GO_chit <- Concentration_f(Solution_long_GO_chit)
...
for(subcomp in unique(Concentration_long_GO_chit$SubCompart)){
  
  # Calculate the summary statistics
    summary_stats_comp <- Concentration_long_GO_chit |>
    filter(SubCompart == subcomp) |>
    filter(Scale == scale) |>
    group_by(Year, Scale, SubCompartName, Unit) |>
    summarise(
      Mean_Concentration = mean(Concentration, na.rm = TRUE),
      SD_Concentration = sd(Concentration, na.rm = TRUE),
      Lower_CI = Mean_Concentration - 1.96 * SD_Concentration / sqrt(n()),
      Upper_CI = Mean_Concentration + 1.96 * SD_Concentration / sqrt(n())
    ) |>
    ungroup()
  
  # Make plot with mean concentration over time and confidence interval band
  concentration_plot <- ggplot(summary_stats_comp, aes(x = Year, y = Mean_Concentration)) +
    geom_line(color = "blue", linewidth = 1) +
    geom_ribbon(aes(ymin = Lower_CI, ymax = Upper_CI), alpha = 0.2, fill = "blue") +
    labs(title = paste("Mean concentration in", subcomp, "with confidence interval over time"),
         x = "Time [year]",
         y = paste0("Concentration [", unique(summary_stats_comp$Unit), "]")) +
    theme_bw()
...
```{r Degradation data}
for(run in unique(Sample_df$RUN)){
  sample_df_run <- Sample_df |>
    filter(RUN == run)
