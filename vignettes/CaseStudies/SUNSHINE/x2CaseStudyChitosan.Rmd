...
Emissions <-
  Emissions |> mutate(Emission_mean_kg_s = Emission_mean_kg_s* Weightfactor,
            ...
            EmisFun = list(
              approxfun(
                data.frame(time_s = c(0,time_s), 
                           emis_kg=c(0,Emission_mean_kg_s)),
                rule = 1:1)
            )
  )
...
emis_kg = c(0,(SBEmissions2 |> filter(Abr == CompartInterest) |> pull(Emission_mean_kg_s)))
...
```{r Output Processing one solution, include=TRUE}
library(dplyr)
#for one Solution
Solution <- as.data.frame(Solution)
Engine <- World$exportEngineR()
...
```{r mass to concentration, include=TRUE}
# Fetch necessary data
Volume <- World$fetchData("Volume")
Area <- World$fetchData("Area")
FRACw <- World$fetchData("FRACw")
...
```{r Concentrations Plotting, include=TRUE}
summed_columns <- list()
