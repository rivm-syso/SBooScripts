...
for(scale in scales){
  ...
  ggsave(paste0(figurefolder, "NR_SBR_Concentration_", scale, ".png"), plot=conc_p, width = 20, height = 10)
  ...
  ggsave(paste0(figurefolder, "NR_SBR_Mass_", scale, ".png"), plot=mass_p, width = 20, height = 10)
}
...
continental_polymer_data <- Concentrations_long |>
  ...
  summarise(Concentration = sum(Concentration)) |>
  group_by(Polymer, Year, SubCompart, SubCompartName, Scale, Unit) |>
  summarise(Concentration = mean(Concentration))

for(subcomp in unique(continental_polymer_data$SubCompart)) {
  plotdata <- continental_polymer_data |>
    filter(SubCompart == subcomp)
  
  cont_pol_plot <- ggplot(plotdata, mapping = aes(x = Year, y = Concentration, color = Polymer)) +
    geom_line() +
    scale_y_continuous(trans = "log10") +
    labs(
      title = paste0("Microplastic concentrations in ", subcomp, " at Continental scale"),
      x = "Year",
      y = paste0("Concentration (", unique(plotdata$Unit), ")")
    ) +
    plot_theme
  
  print(cont_pol_plot)
 
  ggsave(paste0(figurefolder, "Concentration_over_time_Continental_", subcomp, ".png"), plot=mass_p, width = 20, height = 10) 
}
...
ggsave(paste0(figurefolder, "SBR_NR_measurement_comparison",".png"), plot=conc_TNO_SB_SBR_NR, width = 20, height = 10)
