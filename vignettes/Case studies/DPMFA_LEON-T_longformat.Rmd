...
for(i in 1:nrow(unique_combinations)){
  ...
  input_vars <- 
    ...
    mutate(MP_source = replace_na(MP_source, source)) |> 
    ungroup()
...
if(type == "PMFA"){ 
  ...
  for(i in 1:nrow(unique_combinations)){
    ...
    #   mutate(Polymer = pol) |>
    #   mutate(Source = source)
    # solutions$SBdata[[length(solutions)+1]] <- solved
    # if (length(solutions) == 0) {
    #   solutions <- solved  # Initialize with the structure of `solution`
    # } else {
    #   solutions <- rbind(solutions, solved)
    # }
    # 
    
    Output$SBoutput[i] = list(solved)
