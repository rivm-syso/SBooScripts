...
if(MFAtype == "PMFA"){
  ...
  DPMFA_sink_micro <- data_long |>
   ...
   # Load EU data
  abspath <- "vignettes/Case Studies/CaseData/TEST_DPMFA_EU.RData"
  load(abspath)
  ...
  # Load NL data
  abspath <- "vignettes/Case Studies/CaseData/TEST_DPMFA_NL.RData"
  load(abspath)
...
# Path to excel file with distribution values and scale values
...
library(pdqr)
TRWP_data <- 
  TRWP_data |> separate(`Size Fraction (µm)`,
                      into = c("Size_um","max_size_um"), sep = "-")
funProb <- new_d(data.frame(x = as.numeric(TRWP_data$Size_um), y = TRWP_data$PSD_um),
                 type="continuous")
