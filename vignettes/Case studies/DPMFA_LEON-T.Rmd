...
# Assign the emissions to the right compartments. For Tyre wear, emissions go to Attached instead of Solid. 
for(i in sources){
  if(i != "Tyre wear"){
...
for(source in filtersource){
  for(pol in polymers){
    ...
    if(type == "DPMFA"){
      ...
      df_name <- paste0("emis_ss_", source, "_", pol)
...
# Function to process each dataframe
  process_sample_df <- function(sample_df, # data frame with nested samples in data
                                scales, # all SimpleBox scales
                                water, # all SimpleBox water compartments
                                soil_sediment, # all SimpleBox soil and sediment compartments
                                species, # all SimpleBox species
                                small_large, # all small and large SimpleBox names
                                all_SC, # all SimpleBox subcompartment names
                                soil) { # all SimpleBox soil compartments
...
if(type == "PMFA"){ 
  ...
  for(pol in polymers){
    ...
    assign(paste0("solved_ss_", pol), solved)
