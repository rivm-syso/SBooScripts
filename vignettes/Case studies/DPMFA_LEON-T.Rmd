...
data_filtered <- data_summed |>
  ...
  filter(To_Compartment != "Sub-surface soil (micro)") |> # Exclude sub-surface soil because this is outside the scope of SimpleBox
...
for(i in 1:nrow(unique_combinations)){
  ...
  if(type == "DPMFA"){
    ...
    df_name <- paste0("emis_dyn_", source, "_", pol)
    
    assign(df_name, emis_df_dyn)
    
    emis_df_names <- c(emis_df_names, df_name)
    ...
    df_name <- paste0("emis_ss_", source_name, "_", pol)
...
- The mean width of road side soil (2m, based on deliverable 3.2, figure 23)

Zorgen dat er in totaal evenveel landoppervlak is
...
nat_fraction <- areas$Area[2]/(areas$Area[1]+areas$Area[2])
nat_area <- ((sum(areas$Area) - RSS_area_EU))*nat_fraction
agri_area <- sum(areas$Area)-nat_area
...
for(i in 1:nrow(unique_combinations)){
  ...
  if(source == "Tyre wear"){
    ...
    sample_df <- rbind(sample_df, area)
  }
...
if(type == "PMFA"){
  for(name in solved_df_names){
    ...
    for (scale in scales) {
      ...
      ggsave(paste0("vignettes/Case Studies/CaseData/Mass_plot_", scale,  "_", dt, "_", runs, "runs.png"), plot=mass_p)
    }
  }
} else if(type == "DPMFA"){
  for(name in solved_df_names){
...
if(type == "PMFA"){
  for(name in concentration_df_names){
    ...
    for (scale in scales) {
      ...
      ggsave(paste0("vignettes/Case Studies/CaseData/Concentration_plot_", scale, "_", "_", runs, "runs.png"), plot=conc_p)
    }
  }
} else if(type == "DPMFA"){
  for(name in solved_df_names){
