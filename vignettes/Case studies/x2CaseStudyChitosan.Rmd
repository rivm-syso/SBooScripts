...
```{r Example Uncertain Alpha, include = FALSE}
fwa_min <- 1e-4
fwa_max<- 0.1
n <- 100000
...
```
We are also interested in concentrations per compartment. For this, we need to ensure that we convert the Equilibrium Mass into the Equilibrium concentration, which we will do below. We need to adjust the concentrations to end up in respective units. We convert concentrations in soil and sediment to g/kg wet weight. Concentrations in water are in grams/L. In air, concentrations are in kg/kg air. 
```{r}
...
# Print or return the updated Concentration_eq dataframe
print(Concentration_eq)

library(knitr)
library(kableExtra)
subcompart <- c("agriculturalsoil", "naturalsoil", "othersoil", "freshwatersediment", "marinesediment", 
                "air", "deepocean", "river", "sea", "cloudwater")
units <- c("g/kg w", "g/kg w", "g/kg w", "g/kg w", "g/kg w", "g/kg w",
           "kg/kg", "g/L", "g/L", "g/L", "g/L")
...
kable(Concentration_eq, format = "markdown", align = "c", caption = "Concentration per Compartment") |>
  ...
  kableExtra::kableExtraOptions(digits = 4, format = "markdown")
