...
# Step 2: Extract and add data for Chitosan and Graphene Oxide
# Assuming Solution_Chit and Solution_GO are available from the previous processing
for (substance_name in c("Chitosan", "Graphene Oxide")) {
  solution <- if (substance_name == "Chitosan") Solution_Chit else Solution_GO
  
  # Extract the relevant data for each substance
  substance_values <- solution[[interest]]
  time_values <- solution$time
  
  # Combine into a data frame
  substance_data <- data.frame(Time = time_values, Value = substance_values, Run = "Simulated", Substance = substance_name)
  
  # Append to the overall data frame
  variable_data <- rbind(variable_data, substance_data)
}
...
nested_results_df <- bind_rows(nested_results)

# The resulting `nested_results_df` has columns `alpha` and `data`
...
# Iterate over each row in nested_results_df
for (i in seq_len(nrow(nested_results_df))) {
  # Extract the alpha values and results for the current row
  alpha_fw <- nested_results_df$alpha_fw[i]
  alpha_mw <- nested_results_df$alpha_mw[i]
  alpha_results <- nested_results_df$data[[i]]
  ...
  # Process each result for the current alpha combination
  processed_results <- map_df(alpha_results$run, function(run) {
    # Extract the result for the current run
    result <- alpha_results$result[[which(alpha_results$run == run)]]
    
    # Convert the result into a dataframe, transpose it, and adjust column and row names
    ...
    # Return a tibble with the run and the processed result
    tibble(
      alpha_fw = alpha_fw,
      alpha_mw = alpha_mw,
      run = run,
...
final_processed_results_df <- bind_rows(final_processed_results_list)
...
# Iterate over each row in final_processed_results_df
for (i in seq_len(nrow(final_processed_results_df))) {
  # Extract relevant data for the current row
  row_data <- final_processed_results_df[i, ]
  alpha_fw <- row_data$alpha_fw
  alpha_mw <- row_data$alpha_mw
  run <- row_data$run
  print(run)
  degradation <- row_data$degradation[[1]]  # Extract the tibble from the list
  ...
  print("GO done")
  # Store solutions in final results list
  final_results_list[[paste0(run, "_Chitosan")]] <- Solution_Chit
  final_results_list[[paste0(run, "_GO")]] <- Solution_GO
}

# Optionally, if you want to convert the final_results_list to a dataframe
# Convert the list to a dataframe with named elements
final_results_df <- bind_rows(
  lapply(names(final_results_list), function(name) {
    tibble(
      name = name,
      solution = list(final_results_list[[name]])
    )
  })
)
...
```{r using results of uncertain GO-Chitosan for GO and Chitosan with newly added uncertainty}
...
# Initialize an empty list to store the final nested results
final_nested_results <- list()

# Process each combination in the nested_results_df
for (i in seq_len(nrow(nested_results_df))) {
  # Extract the current alpha combination and nested data
  alpha_fw <- nested_results_df$alpha_fw[i]
  alpha_mw <- nested_results_df$alpha_mw[i]
  nested_data <- nested_results_df$data[[i]]
  
  # Initialize lists to store results for each substance
  results_Chitosan <- list()
  results_GO <- list()
  
  for (run in unique(nested_data$run)) {
    # Extract the data for the current run
    run_data <- nested_data %>%
      filter(run == !!run)
    
    # Apply the degradation calculations
    degradation_remaining <- run_data$result[[1]] %>%
      mutate(across(where(is.numeric), ~ . * 0.85))
    degradation_GO <- run_data$result[[1]] %>%
      mutate(across(where(is.numeric), ~ . * 1/8))
    degradation_Chit <- run_data$result[[1]] %>%
      mutate(across(where(is.numeric), ~ . * 7/8))
    ...
    # Store solutions for the current run
    results_Chitosan[[run]] <- Solution_Chit
    results_GO[[run]] <- Solution_GO
  }
  
  # Combine results for Chitosan and Graphene Oxide into a single dataframe
  final_results <- tibble(
    Chitosan_results = list(bind_rows(
      map2(names(results_Chitosan), results_Chitosan, ~ tibble(run = .x, result = list(.y)))
    )),
    GO_results = list(bind_rows(
      map2(names(results_GO), results_GO, ~ tibble(run = .x, result = list(.y)))
    ))
  )
  
  # Nest the results for the current alpha combination
  final_nested_results[[i]] <- tibble(
    alpha_fw = alpha_fw,
    alpha_mw = alpha_mw,
...
# Combine all alpha results into a single nested tibble
final_nested_results_df <- bind_rows(final_nested_results)
