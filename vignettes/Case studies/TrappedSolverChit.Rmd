...
kaas_time <- kaas_time |>
  ...
  # Step 2: Make values negative if 'from' and 'to' abbreviations are the same
  mutate(across(where(is.numeric), ~ if_else(from == to, -., .)))

removal <- kaas_time |>
  # Filter rows where 'from' and 'to' columns are the same
  filter(from == to)
...
columns_to_keep <- names(degradation_Chit) |>
  sapply(is_numeric_name) |>
  `|`(names(degradation_Chit) == 'from') |>
  names(degradation_Chit)[.]
...
emissions_chit <- as.data.frame(t(degradation_Chit_filtered))
colnames(emissions_chit) <- solution_transposed[, ]
Emissions_Chit <-
  group_by(from) |> 
  summarise(n=n(),
            EmisFun = list(
