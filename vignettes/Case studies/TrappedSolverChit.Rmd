...
```{r new initialisation of World}
World$substance <- "Chitosan"
...
numeric_columns <- sapply(names(degradation_Chit), is_numeric_name)

# Combine this logical vector with the condition for the column 'from'
keep_columns <- numeric_columns | names(degradation_Chit) == 'from'

# Get the column names to keep
columns_to_keep <- names(degradation_Chit)[keep_columns]

# Select only these columns
degradation_Chit_filtered <- degradation_Chit |>
  select(all_of(columns_to_keep))

long_filtered <- degradation_Chit_filtered %>%
  pivot_longer(cols = -from, names_to = "time", values_to = "value")

Emissions_Chit <- long_filtered |>
  group_by(from) |> 
  summarise(n=n(),
            EmisFun = list(
              approxfun(
                data.frame(time_s = c(0,time), 
                           emis_kg=c(0,value)),
                rule = 1:1)
            )
  )
funlist_Chit <- Emissions_Chit$EmisFun
names(funlist_Chit) <- Emissions_Chit$from
CompartmentInterestChit <- "w1CS"
time_s = c(0,(long_filtered |> filter(from == CompartmentInterestChit) |> pull(time)))
emis_kg = c(0,(long_filtered |> filter(from == CompartmentInterestChit) |> pull(value)))

PlotEmisFun = funlist_Chit[[CompartmentInterestChit]]
...
```{r re-initialization of the ODE solver}
rm(Solution)
Solution_Chit <- SBsolve4(tmax = 24*(365.25*24*3600),
nTIMES = 130,
Engine = World$exportEngineR(),
