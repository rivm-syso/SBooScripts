...
We also want to include uncertainty within our parameters. In this case, we examine the influence of different attachment efficiencies (alpha's) We create combinations with LHS and base this on a log-normal distribution to represent a more truthful representation of attachment efficiencies in nature. 
...
In this chunk we start solving for GO-Chitosan
...
```
We plot the results for GO-Chitosan below. We create uncertaincy bands around our mean prediction based on Confidence Intervals. 
...
for (i in seq_len(nrow(nested_results_df_GoChit))) {
  ...
  for (j in seq_len(nrow(nested_data))) {
    ...
    cat("Length of variable_values:", length(variable_values), "\n")
    cat("Length of time_values:", length(time_values), "\n")
    cat("Sample variable_values:", head(variable_values), "\n")
    cat("Sample time_values:", head(time_values), "\n")
...
for (i in seq_along(combinations_list)) {
  ...
  # Initialize a list to store results for the current alpha
  alpha_results <- list()
  ...
  for (Run in unique_runs) {
    ...
    # Store results for the current run using named list indexing
    alpha_results[[as.character(Run)]] <- tibble(
      run = Run,
...
mass_results_df <- nested_results_df_GOandChit %>%
  select(alpha_fw, alpha_mw, mass_data) %>%
  unnest(cols = c(mass_data))

concentration_results_df <- nested_results_df_GOandChit %>%
...
```
## final plots 
We now plot the final results for all three substances. 
```{r final plots}
  ...
  for (i in seq_len(nrow(nested_results_df))) {
    ...
    for (j in seq_len(nrow(nested_data))) {
      ...
      result <- if(substance_name == "GO-Chitosan") {
        nested_data$mass[[j]]  # Extract GO-Chitosan result
