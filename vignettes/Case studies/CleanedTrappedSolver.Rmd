...
# Step 2: Calculate mean and confidence intervals at each time point across all alphas, convert to ug/L
summary_stats_concentration <- all_concentration_data |>
  group_by(Time, Substance) |>
  summarise(
    Mean_Value = mean(Value, na.rm = TRUE) *1e3,
    SD_Value = sd(Value, na.rm = TRUE) * 1e3,
    Lower_CI = Mean_Value - 1.96 * SD_Value / sqrt(n()),
    Upper_CI = Mean_Value + 1.96 * SD_Value / sqrt(n())
  ) |>
  ungroup()
...
# Step 
ggplot(summary_stats_concentration, aes(x = Time/(365.25*24*3600), y = Mean_Value, color = Substance)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = Lower_CI, ymax = Upper_CI, fill = Substance), alpha = 0.2) +
