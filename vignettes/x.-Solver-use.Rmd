...
load("data/Example/example_uncertain_data.RData")
...
emissions <- 
  ...
  data.frame(Emis = c(10,20, 1,0,0), # emission in tones per year
             Timed = c(1,5,10,15,20))  # years from start
...
# Abbr = c("aRS", "s2RS", "w1RS","aRS", "s2RS", "w1RS","aRS", "s2RS", "w1RS")

# convert 1 t/y to si units: kg/s
emissions <- emissions |>
  mutate(Emis = Emis*1000,
    Timed = Timed*(365.25*24*60*60)) |> ungroup()
```
...
tmax <- max(emissions$Timed) # set max solve time to last step in emission scenario
nTIMES <- 1+max(emissions$Timed)/(365.25*24*60*60) # Sets the time step for output, e.g. for 20 year scenarion, add t0 is 21 nTimes
