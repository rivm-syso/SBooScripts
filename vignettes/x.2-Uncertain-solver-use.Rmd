...
p3_data <- 
  Input_Emission |> pivot_wider(names_from = Abbr, names_glue = "{Abbr}_Emis",
                                values_from = value) |> 
  full_join(solved$SteadyStateMass) |>   filter(SubCompart == "river") |> 
  pivot_longer(c(aRU_Emis,s2RU_Emis), values_to = "Emission",names_to = "EmisComp")

p3 <-  ggplot(p3_data, mapping = aes(x=Emission, y = EqMass)) +
