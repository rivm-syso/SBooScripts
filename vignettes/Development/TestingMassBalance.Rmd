...
process_substance <- function(data, substance_name) {
  ...
  plot <- ggplot(filtered_data, aes(x = process, y = k, fill = process)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
   geom_text(aes(label = paste("Difference:", round(difference, 2))), 
...
kaas_filtered_list <- list()
  ...
  # Replace with actual emissions data or parameters
...
ggplot(total_abs_differences, aes(x = as.factor(VaporPressure), y = TotalAbsDifference)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Total Absolute Differences by Vapor Pressure", x = "Vapor Pressure", y = "Total Absolute Difference [kg]") +
  theme_minimal()

# Plotting the mass balance differences
ggplot(massbalance_difference_all, aes(x = interaction(Scale, SubCompart, Species), y = difference, fill = as.factor(VaporPressure))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Mass Balance Differences by Scale, SubCompart, and Species", x = "Combination (Scale, SubCompart, Species)", y = "Difference Mass Balance [kg]") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_minimal()

top_rows_all$VaporPressure <- factor(top_rows_all$VaporPressure, levels = unique(top_rows_all$VaporPressure))

# Plotting the top differences as grouped bars with facets for vapor pressures
ggplot(top_rows_all, aes(x = interaction(SubCompart, Scale, Species), y = difference, fill = as.factor(VaporPressure))) +
  geom_bar(stat = "identity", position = "dodge", width = 0.4) +
  labs(title = "Top Differences by Compartment, Scale, and Species", x = "Combination (Compartment, Scale, Species)", y = "Difference Mass Balance [kg]", fill = "Vapor Pressure") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  facet_wrap(~ VaporPressure, scales = "free", ncol = 2)
