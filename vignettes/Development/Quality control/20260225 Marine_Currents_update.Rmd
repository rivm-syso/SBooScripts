...
-   Compare marine_currents_clean with Remove_global = TRUE and Regional_and_Continental_deepocean = TRUE with the development branch
...
```{R Plots development and marine currents with vars as true, include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, fig.width=9, fig.height=6}
...
```{R Plots development and marine currents with vars as false, include=TRUE, echo=TRUE, message=FALSE, warning=FALSE, fig.width=9, fig.height=6}
diffs <- kaas_comparison |>
  mutate(fromname = paste0(process, "_", fromSubCompart, "_", fromScale)) |>
  mutate(toname = paste0(process, "_", toSubCompart, "_", toScale)) |>
  mutate(diff = ifelse(diff == 0, NaN, diff)) |>
...
When both test values are turned off in the marine_currents_clean branch, the results are the same as in the development.
