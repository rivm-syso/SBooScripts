  ...
  for (solver in solvers) {
    ...
    kaas <- kaas |>
      mutate(k = if_else(fromSubCompart == toSubCompart &
                         ...
                         k))
    kaas_filtered <- kaas |>
      filter((fromScale == "Tropic" & fromSubCompart == "marinesediment" & fromSpecies  == "Small") |
             (toScale == "Tropic" & toSubCompart == "marinesediment" & toSpecies == "Small"))
...
process_substance <- function(data, substance_name) {
  ...
  mutate(
    condition_met = fromSubCompart == "marinesediment" & fromScale == "Tropic" ,
  ...
  plot <- ggplot(filtered_data, aes(x = process, y = k, fill = process)) +
    ...
    theme_minimal() +
    labs(
      title = paste("k values for", substance_name, "and", solver," for marinesediment at tropical Scale"),
      x = "net Process",
      y = "k [k*EqMass]"
    ) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
