...
for(i in 1:nrow(substances)){
  ...
  if(is.na(main_solution)) main_solution <- solution else {
    merge(main_solution,solution)}
...
for(i in 1:nrow(substances)){
  ...
  if(is.na(test_solution)) test_solution <- solution else {
    merge(test_solution,solution)}
...
ggplot(solution_comparison, mapping = aes(x = Abbr, y = diff)) + 
...
ggplot(solution_comparison, mapping = aes(x = Abbr, y = rel_diff)) + 
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(
