---
title: "General script comparing plastic_fade_degradation branches to development"
...
```{R Specify temporary directory, include = FALSE, echo=TRUE, message=FALSE, warning=FALSE}
# Specify where the downloaded versions of sboo and sbooscripts should be saved to
dest_folder <- "SimpleBox"
```

```{R Specify which tag and branch to compare, echo=TRUE, message=FALSE, warning=FALSE}
...
```{R Clean environment again, include=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
rm(list = ls()[sapply(ls(), function(x) is.function(get(x)))])
```

```{R Run SB and get advection ks for plastic_fade_degradation branch, results ="hide", echo=TRUE, message=FALSE, warning=FALSE}
...
The differences found for neutral and acidic substances are related to rounding differences between the Substances.csv in the development branch and the plastic_fate_degradation branch. 

The updates made to the degradation rate in the plastic_fade_branch cause the changes visible in the plot with mean relative difference for particulates; only k_Degradation rates were changed. All degradation rates are 50% higher than before. 
