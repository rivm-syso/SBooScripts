2026.4.0 Adjusting marine currents
================
...
- Compare release 2026.4.0 with Remove_global = TRUE and
  Regional_and_Continental_deepocean = TRUE with the release 2026.3.1
  branch

- Compare release 2026.4.0 with Remove_global = FALSE and
...
## Calculate first order rate constants for release 2026.3.1

``` r
# Save the original working directory for later
original_wd <- getwd()
# Change the wd to the wd of release 2026.3.1
...
for(i in 1:nrow(substances)){
  ...
  World$SetConst(Remove_global = "TRUE") #if true, remove flows from, to and within moderate, arctic and tropic scales
...
![](2026.3.1_comparison_with_Marine_currents_files/figure-gfm/Plots%20release%20and%20marine%20currents%20with%20vars%20as%20true-9.png)<!-- -->![](2026.3.1_comparison_with_Marine_currents_files/figure-gfm/Plots%20release%20and%20marine%20currents%20with%20vars%20as%20true-10.png)<!-- -->
    ...
    ## [1] "No k's changed for particle substances"

## Discussion

From the plots we can see the following: - release 2026.4.0 turns the
advection flow from Regional sea to Continental sea off for all
species - All deposition rates from air to water and soil compartments
are slightly different for molecules (neutral, no class, base, acid,
metal) v_Otherkair is a variable used in k_Deposition. Otherkair is
always NA for particulates, but for molecules Otherkair equals the sum
of all k’s from the same Scale-Species combination where the SubCompart
is air. Because changes in Advection processes (removal of global
scales) changes the other k’s to the air compartment at global and
continental scales, this causes changes in k_Deposition.
...
for(i in 1:nrow(substances)){
  ...
  # World$SetConst(Remove_global = "FALSE") #if true, remove flows from, to and within moderate, arctic and tropic scales
  # World$UpdateKaas(mergeExisting = FALSE)
...
```
    ...
    ## [1] "No k's changed for neutral substances"

    ## [1] "No k's changed for  substances"

    ## [1] "No k's changed for acid substances"

    ## [1] "No k's changed for base substances"

    ## [1] "No k's changed for metal substances"
...
When both test values are turned off in release 2026.4.0, the results
are the same as in the release 2026.3.1.
