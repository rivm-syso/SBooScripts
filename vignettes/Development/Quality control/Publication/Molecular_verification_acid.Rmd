...
*At release already improvements or developments have been implemented
in the R version of SimpleBox (SBoo) which are not implemented in Excel
...
*In the chunk below we check if our matrix is as large as we expect it
to be.*
...
*With these exceptions, we expect our matrix to be 60- 25 = 35 rows.
This script will stop running if this is not the case.*

ii) *We expect all processes to be included in Processes4SpeciesTp.csv
    to also be calculated. Therefore, we compare the calculated unique
...
*At the time of this verification, some improvements were already made
in the R version versus the excel version. This meant that some k's
differ between R and excel, but not because the calculations or input
values are wrong. In order to still be able to compare the two versions,
the 'Test' variable was created. This variable is a boolean that can be
...
*The dynamic solver can use time explicit emissions and provide masses
or concentration in time. This shows the increase in mass in each
compartment in time based on a specified emission, see
[here](vignettes/10.0-Solver-use.md "Solver documentation") for more
details. Here we verify that with constant emission in time the steady
state mass is approached.*

```{r}

emissions <- 
  # create emission scenario in time:
  data.frame(Emis = c(10000,10000,0,0), # emission in kg/year
             Time = c(0,100,110,150))  # years from start
# apply emission scenario to intended species and compartments using hte Abbr:
emissions <- merge(emissions, data.frame(Abbr = c("aRU", "s2RU", "w1RU")))
...
SSsolveD.R <- 
  ...
  mutate(Abbr = str_replace(Abbr, "sd0R", "w0R")) |>
  mutate(Abbr = str_replace(Abbr, "sd0C", "w0C")) |>
  group_by(Scale, SubCompart, Species, Abbr,time) |>
  summarise(Dynamic_Mass_kg = sum(Mass_kg))

# calculate time to steady state
Time2SteadyState <- 
  SSsolveD.R |> 
  left_join(SSsolve.R |> 
              mutate(SS_Mass_kg=Mass_kg,
                     .keep = "unused")) |> 
...
barplot(SS_time_y ~ Abbr, data=Time2SteadyState, las = 2)
...
if(length(unique(Time2SteadyState$Abbr)) != length(unique(SSsolve.R$Abbr))){
  warning(paste("Not all compartments reaching steady state within",max(as.numeric(SSsolveD.R$time))/(365.25*24*60*60),"years"))
}
 ...
 SSsolveD.R |> 
  left_join(SSsolve.R |> 
              mutate(SS_Mass_kg=Mass_kg,
                     .keep = "unused")) |> 
  mutate(FractionSS = Dynamic_Mass_kg/SS_Mass_kg) |> 
  filter(FractionSS > 0.999)

test_dynamic_2 <- test_dynamic_1 |> 
   filter(FractionSS > 1.001)

if(length(test_dynamic_2$Abbr)>0){
  stop(paste0("Compartment ",test_dynamic$Abbr,"exceeding steady state"))
}
