...
emissions <- data.frame(Abbr = c("aRU", "s2RU", "w1RU","aAU","w2AU"), 
                        Emis = c(10000, 10000, 10000,100,100)) # t/y

MW <- World$fetchData("MW")

emissions <- emissions |>
  mutate(Emis = Emis * 1000 / (365 * 24 * 60 * 60)) # convert 1 t/y to si units: kg/s
...
in now only
`r merged_SS_SB1$reldiff |> abs()|> max() |> round(digits=4)*100`%. This
proves that the port of SimpleBox to R is successful in reproducing the
results from the original spreadsheet implementation for chemicals of
