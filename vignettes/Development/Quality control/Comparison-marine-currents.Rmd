---
title: "Comparison marine currents" author: "Anne Hids, Joris Quik" date:
...
```{R Run SB and get advection ks for marine_currents_clean branch, results ="hide", echo=TRUE, message=FALSE, warning=FALSE}
# Change the wd to the wd of the main branches
...
There is no need to run SB again for the main, we already have the main k's for the substances we want to compare.
## Calculate the first order rate constants for the test branches
...
```{R}
main_combo <- main_kaas %>% distinct(fromScale, fromSubCompart)
test_combo <- test_false_kaas %>% distinct(fromScale, fromSubCompart)

main_combo <- main_combo %>% mutate(source = "main")
test_combo <- test_combo %>% mutate(source = "test")

all_combos <- bind_rows(main_combo, test_combo)

# Tel per combinatie hoe vaak deze voorkomt (in main, test, of beide)
combo_check <- all_combos %>%
  group_by(fromScale, fromSubCompart) %>%
  summarise(
    in_main = any(source == "main"),
    in_test = any(source == "test"),
    .groups = "drop"
  )

#Alleen in main:
only_in_main <- combo_check %>% filter(in_main & !in_test)
#Alleen in test:
only_in_test <- combo_check %>% filter(!in_main & in_test)
#In beide:
in_both <- combo_check %>% filter(in_main & in_test)
...
Exceptions should be added to the functions printed above. 
