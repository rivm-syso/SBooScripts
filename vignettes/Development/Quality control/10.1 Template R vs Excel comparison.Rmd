...
kaas <- filter(kaas, from != to) #filtering the diagonals out
kaas2 <- kaas %>% group_by(from, to) %>% summarize(k = sum(k)) #R version sometimes has multiple k's per fromto box; excel only has the summed k's per box
