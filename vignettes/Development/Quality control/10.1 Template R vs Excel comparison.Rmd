...
> diagonal$from[!diagonal$from %in% diagonal_excel$from]
 [1] "s2A"  "s2M"  "s2T"  "s3A"  "s3M"  "s3T"  "sd0A" "sd0C" "sd0M" "sd0R" "sd0T" "sd1A" "sd1M" "sd1T" "w0A"  "w0M"  "w0T"  "w1A"  "w1M"  "w1T" 
[21] "w3C"  "w3R" 


This is due to the degradation rate constant being calculated for all combinations of subcompartments. There is currently no data sheet where the exclusion based on combination of subcompartment and scale can be set. So these types of exclusions are needed in the process function itself.

So now the compartments are the correct amount based on fixing exclusions in k_Degradation.
...
Some testing needed to see why K's from air are so different from those in Excel.
