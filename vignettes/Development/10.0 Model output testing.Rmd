...
summedkaas <- aggregate(k ~ from + to, data = kaas, FUN = sum) #summing k's in the same "box". Need to check if this is the right thing to do

diagonal_kaas <- summedkaas[summedkaas$from == summedkaas$to,] #diagonals need some more attention, formula is different in excel. probably need to substract all the "froms" from that box.  
diagonal_excel <- SBexcel.K[SBexcel.K$from == SBexcel.K$to,] #all the diagonals in excel are negative values
