...
`r round(max(abs(unique(changed_kaas$rel_diff)))*100, 1)` % between the
original and Bagheri drag methods. This affects the
`r changed_kaas |> distinct(process) |> pull()` processes.

`r knitr::kable(changed_kaas)`

From the above table it can be seen, that also the Ag 10 nm nanomaterial
is affected with an
`r``round(max(abs(unique(changed_kaas$rel_diff)))*100, 1)` decrease in
k_sedimentation. It is (for now) advised to not use the Bagheri drag
method for nanomaterials.

```{R echo=FALSE, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
all_diffs <- kaas_comparison |>
  filter(abs(rel_diff) > 0.05) |> 
  mutate(fromname = paste0(fromSubCompart, "_", fromScale, "_",fromSpecies)) |>
  mutate(toname = paste0(toSubCompart, "_", toScale, "_", toSpecies))
...
```{R Run SB and get k values for different shapes, message=FALSE, warning=FALSE, include=FALSE}
