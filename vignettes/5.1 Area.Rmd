...
Instead of hardcoding the nesting and structure, a ScaleNestGroup and
the exising ScaleOrder are applied to calculate the AreaSea and
AreaLand.

For this the variable `ScaleNestGroup` is introduced. It should be the
same number for each scale that can is nested within each other, the
nesting group. In the default scenario this is (Local), Regional,
Continetnal and Moderate scales being nested.
