...
for (i in 1:n_vars) {
  ...
  if (UncertParams$Distribution[i] == "Triangular") {
    samples <- triangular_cdf_inv(lhs_samples, a, b, c)
  }
  if (UncertParams$Distribution[i] == "Normal") {
    samples <- normal_pdf(lhs_samples, b, c)
    samples[samples <= 0] <- a
  }
  if (UncertParams$Distribution[i] == "Log normal") {
    samples <- LogNormal_pdf(lhs_samples, b, c)
    samples[samples <= 0] <- a
  }
  if (UncertParams$Distribution[i] == "Weibull") {
    samples <- Weibull_pdf(lhs_samples, a, b, c)
  }
...
for (i in 1:24) {
  a <- ifelse(is.na(MEC_In$a[i]), 1e-10, MEC_In$a[i])  
  b <- MEC_In$b[i]
  c <- MEC_In$c[i]
...
PECMEC <- tibble(Substance = Conc_calc$Substance,
                  ...
                  "PEC:MEC" = Conc_calc$Value/MEC_Distributed$Value)
...
for (Substance in Substances) {
  for (SubCompart in Subcomparts){
    ...
    if (!is.na(index[1])) {
      ...
      PECMEC_min <- min(PECMEC$`PEC:MEC`[index])
      PECMEC_quant25 <- quantile(PECMEC$`PEC:MEC`[index], 0.25)
      PECMEC_median <- median(PECMEC$`PEC:MEC`[index])
      PECMEC_quant75 <- quantile(PECMEC$`PEC:MEC`[index], 0.75)
      PECMEC_max <- max(PECMEC$`PEC:MEC`[index])
