...
LogNormal_cdf_inv <- function(u, a, b){
  1 / (u*log(b)*sqrt(2*pi)) * exp(-(log(u-log(a)))^2 / (2*log(b)^2))
}
...
var1 <- World$fetchData(var1Name) |>
  ...
  mutate(SubCompart = NA, Dist = "Triangle")
...
var2$a <- var2$CORG.susp  # peak value (peak)
var2$b <- 0.04               # Maximum value
var2$c <- 0
...
var7 <- World$fetchData(var7Name) |>
  ...
  mutate(Scale = NA, Dist = "LogNormal")

var7$a <- var7$SUSP     # peak value (peak)
var7$b <- 0.0235              # Maximum value
var7$c <- 0
...
var8$a <- var8$Corg     # peak value (peak)
var8$b <- 0.04               # Maximum value
var8$c <-0
...
var9$a <- var9$Corg     # peak value (peak)
var9$b <- 0.04               # Maximum value
var9$c <- 0
...
params <- tibble(
  ...
  SubCompart = c(var1$SubCompart, var2$SubCompart, var3$SubCompart, var4$SubCompart, var5$SubCompart,
                 ...
                 var16$SubCompart),
  Distribution =c(var1$Dist, var2$Dist, var3$Dist, var4$Dist, var5$Dist,
                  var6$Dist, var7$Dist, var8$Dist, var9$Dist, var10$Dist,
                  var11$Dist, var12$Dist, var13$Dist, var14$Dist, var15$Dist,
                  var16$Dist),
  data = list(
...
for (i in 1:n_vars) {
  ...
  if (params$Distribution[i] == "Normal") {
    samples <- normal_cdf_inv(lhs_samples[, i], a, b)
  }
  if (params$Distribution[i] == "LogNormal") {
    samples <- LogNormal_cdf_inv(lhs_samples[, i], a, b)
  }
