...
for (i in 1:10) {
  a <- ifelse(is.na(MEC_In$a[i]), 1e-15, MEC_In$a[i]*1e-6)  
  b <- MEC_In$b[i]*1e-6
  c <- MEC_In$c[i]*1e-6
...
for (Substance in Substances) {
  ...
  GSA_table <- add_column(GSA_table, "{paste0('Concentration_',Substance)}" := Conc_calc$Value[which(Conc_calc$Substance == Substance)])
  #write.xlsx(GSA_table, "/rivm/n/defaresj/Documents/GSA_table.xlsx")
  ...
  probX <- probX_Y |> 
    mutate("{paste0('Concentration_', Substance)}" := NULL) |> 
    data.matrix()
  probY <- probX_Y |> 
    pull(paste0('Concentration_', Substance) )
  ...
  Sens_idices <- add_column(Sens_idices, "{Substance}" := borg_d_temp$delta)
}
